...
$sql = "
  ...
  LIMIT 3
";
foreach ($pdo->query($sql) as $row) $popular[] = $row['name'];
?>
<!doctype html>
<html>
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .chip{background:var(--chip); border:1px solid var(--ring); padding:6px 10px; border-radius:999px; font-size:13px; color:#cbd5e1}
    .actions{grid-column: span 12; display:grid; gap:12px; grid-template-columns: repeat(4, 1fr)}
    .btn{display:block; text-align:center; padding:12px 14px; border-radius:12px; background:#111827; border:1px solid var(--ring); color:#e5e7eb; text-decoration:none; font-weight:600; transition:.15s transform,.15s background}
    .btn:hover{transform:translateY(-1px); background:#0f172a}
    .btn:active{transform:translateY(0)}
    .btn svg{vertical-align:-3px;margin-right:6px}
    @media (max-width:800px){ .stat{grid-column: span 12} .actions{grid-template-columns: repeat(2, 1fr)} }
  </style>
</head>
<body>
  <div class="container">
    <header>
      ...
      <div>
        <a href="/index.php" style="color:#cbd5e1;text-decoration:none">← Back to site</a>
      </div>
    </header>
    ...
    <section class="grid">
      <div class="card stat accent-1">
        ...
        <div class="value"><?= (int)$activeStudents ?></div>
      </div>
      ...
      <div class="card stat accent-3">
        ...
        <div class="value"><?= number_format((float)$avgCredits, 2) ?></div>
      </div>

      <div class="card list">
        <div class="label">Most Popular Skills (offered)</div>
        <div class="chips">
          <?php if ($popular): foreach ($popular as $name): ?>
            <span class="chip"><?= h($name) ?></span>
          <?php endforeach; else: ?>
      ...
      <div class="actions">
        <a class="btn card" href="/admin/students.php">
        ...
        <a class="btn card" href="/admin/credits.php">
        ...
        <a class="btn card" href="/admin/skills.php">
        ...
        <a class="btn card" href="/admin/moderation.php">
